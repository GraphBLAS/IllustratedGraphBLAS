...
class Scene7(VoiceoverScene, Scene):
    ...
    def construct(self):
        ...
        with self.voiceover(
            ...
            lower_blocks = VGroup(select_group, apply_group)
            lower_blocks.arrange(RIGHT, buff=1)
            lower_blocks.next_to(blocks, DOWN, buff=0.8)

            self.play(FadeIn(select_group), FadeIn(apply_group))
            self.wait(1)
