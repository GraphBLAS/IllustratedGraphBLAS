...
class Scene3(VoiceoverScene, Scene):
    ...
    def construct(self):
        ...
        # S and D preserve edge identity (3 nodes × 4 edges)
        S_data = [
            [1, 0, 1, 0],  # node 0 sources e0, e2
            [0, 1, 0, 0],  # node 1 sources e1
            [0, 0, 0, 1],  # node 2 sources e3
        ]
        ...
        with self.voiceover(
            ...
            # Add row/col labels
            A_row_labels = VGroup(*[
                Text(str(i), font_size=14, color=BLUE).next_to(A_mat.get_rows()[i], LEFT, buff=0.45)
                for i in range(3)
            ])
            A_col_labels = VGroup(*[
        ...
        with self.voiceover(
            ...
            # Entry (i, j) counts when the destination of edge i is the source
            # of edge j — i.e., the two edges chain together via a shared node.
            DS_note = VGroup(
        ...
        with self.voiceover(
            ...
            summary = VGroup(adj_section, sd_section).arrange(
